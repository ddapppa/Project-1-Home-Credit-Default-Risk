...
│ ├── 04_modeling.ipynb
│ ├── 05_evaluation.ipynb
│ ├── 06_cleaning_auxiliary.ipynb # Cleaning v2: tabel auxiliary (bureau, previous_application, dst)
│ ├── 07_feature_engineering_v2.ipynb # Feature engineering v2: menggabungkan tabel auxiliary
│ ├── 08_modeling_v2.ipynb # Modeling v2: retrain dengan fitur v2
│ ├── 09_feature_engineering_v3.ipynb # Feature engineering v3: refinement fitur final
│ ├── 10_modeling_v3.ipynb # Modeling v3: model final (CatBoost v3)
│ └── 11_evaluation.ipynb # Evaluasi & perbandingan akhir seluruh model
...
│ ├── predict.py
│ └── evaluate.py
├── models/ # 14 model dibandingkan sepanjang iterasi v1-v3
│ ├── logistic_regression/
│ ├── decision_tree/
│ ├── random_forest/
│ ├── lightgbm/ lightgbm_v2/ lightgbm_v3/
│ ├── xgboost/ xgboost_v2/ xgboost_v3/
│ ├── catboost/ catboost_v2/ catboost_v2_tuned/ catboost_v3/ # Model final
│ └── stacking_meta_v3/ # Ensemble stacking dari model-model terbaik
├── docs/
│ └── explainability.md # Detail lengkap SHAP & interpretability model
├── reports/
│ ├── model_summary.csv # Ringkasan metrik seluruh model yang dibandingkan
│ └── figures/ # Seluruh visualisasi EDA, evaluasi, dan demo.gif
...
git clone <repo-url>
cd Project-1-Home-Credit-Default-Risk-main
...
**Model final:** CatBoost v3 (pipeline tunggal, dipilih setelah membandingkan **14 model** — Logistic Regression, Decision Tree, Random Forest, LightGBM v1-v3, XGBoost v1-v3, CatBoost v1-v3 (tuned & final), MLP, dan Stacking Meta-model — lihat `notebooks/11_evaluation.ipynb` untuk perbandingan lengkap dan `reports/model_summary.csv` untuk ringkasan metrik semua model).
